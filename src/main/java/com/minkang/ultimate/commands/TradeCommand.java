...
public class TradeCommand implements CommandExecutor {
    ...
    private void showTradeHelp(CommandSender s){
        String bar = "§8§m────────────────────────────────";
        s.sendMessage(bar);
        s.sendMessage("§b/거래 §7<플레이어> §8— §f거래 요청");
        s.sendMessage("§b/거래 수락 §8— §f들어온 요청 수락");
        s.sendMessage("§b/거래 취소 §8— §f진행 중/대기 중 거래 취소");
        s.sendMessage(bar);
    }
