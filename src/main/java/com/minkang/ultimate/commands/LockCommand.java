...
public class LockCommand implements CommandExecutor {
    ...
    private void showLockHelp(CommandSender s){
        String bar = "§8§m────────────────────────────────";
        s.sendMessage(bar);
        s.sendMessage("§b/잠금 §7설치 §8— §f현재 바라보는 상자/문에 잠금 설치");
        s.sendMessage("§b/잠금 §7영구 §8— §f영구 잠금 설치(토큰 소모)");
        s.sendMessage("§b/잠금 §7추가 <닉네임> §8— §f공유자 추가");
        s.sendMessage("§b/잠금 §7목록 §8— §f공유자 목록 보기");
        s.sendMessage("§b/잠금 §7해제 §8— §f잠금 해제 안내");
        s.sendMessage(bar);
    }
