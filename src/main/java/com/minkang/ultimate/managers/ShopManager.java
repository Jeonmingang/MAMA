...
public class ShopManager implements Listener {
    ...
    /** 손에 든 아이템을 특정 슬롯에 등록 */
    public boolean addItemToSlot(org.bukkit.entity.Player p, String name, int slot, boolean allowBuy, boolean allowSell, Double price){
        try{
            org.bukkit.inventory.ItemStack inHand = p.getInventory().getItemInMainHand();
            if (inHand == null || inHand.getType() == org.bukkit.Material.AIR) return false;
            FileConfiguration cfg = plugin.getConfig();
    ...
    public java.util.Map<Integer,String> getAllBindings(){
        return new java.util.HashMap<>(npcBindings);
    }
